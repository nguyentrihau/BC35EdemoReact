...
export default class ExRenderFilms extends Component {
  renderCardFilms = () => {
    return arrFilms.map((prod, index) => {
      return <div className="col-2 mt-2" key={index}>
        <div className="card">
          <img src={prod.hinhAnh} alt="..." height={350} style={{objectFit:'cover'}} />
          <div className="card-body bg-dark text-white">
            <p style={{height:50}} >{prod.tenPhim}</p>
            <p className='' style={{height:110}}>{prod.moTa.length > 100 ? prod.moTa.substring(0,100) + "..." : prod.moTa}</p>
          </div>
        </div>
      </div>
    })
