...
export default class DanhSachSanPham extends Component {
    render() {
        const {themGioHang,xoaGioHang}=this.props;
        return (
            <div>
                <h3 className='mt-2 text-center'>Danh sach san pham</h3>
                <div className="row">
            {
                data.map((item,index)=>{
                    return <div className="col-4" key={index} >
                        <SanPham prod={item} themGioHang={themGioHang} xoaGioHang={xoaGioHang}/>
                    </div>
                })
            }
                </div>
